    ...
    <div className="mx-auto grid w-full max-w-[760px] gap-4">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-medium leading-tight text-ink">Review and pay</h1>
        </div>
      ...
      <section className="tr-card p-4">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            ...
            {rows.length > 0 ? (
              <div className="mt-3 grid gap-2">
                {rows.map((row) => (
                  <div key={`${row.item.productId}-${row.option.id}-${row.item.setup?.destinationUrl ?? ""}`} className="grid gap-2 rounded-md border border-line bg-white p-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
                        <p className="text-sm font-medium leading-5 text-ink">{row.product.title}</p>
                        <p className="text-xs text-brand">{row.option.label}</p>
                      </div>
                      <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-xs leading-5 text-muted">
                        {row.item.setup?.businessName ? <span>Business: {row.item.setup.businessName}</span> : null}
                        {row.item.setup?.destinationUrl ? <span className="break-all">Destination: {row.item.setup.destinationUrl}</span> : null}
                        {row.item.setup?.serviceMode === "HOSTED" && row.item.setup?.serviceAddon === "hosted_multilink" ? (
                          <span>Multi-Link: {formatPrice(row.item.setup.monthlyPriceCents ?? 0)}/mo</span>
                        ) : null}
                      </div>
                    </div>
                    <div className="text-sm font-medium text-ink sm:text-right">
          ...
          <div className="grid min-w-[210px] gap-2 rounded-md bg-soft p-3 text-sm">
