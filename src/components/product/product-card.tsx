...
export function ProductCard({ product, density = "default" }: { product: MigratedProduct; density?: "default" | "compact" }) {
  ...
  return (
    ...
    >
      ...
      <div className={isCompact ? "flex min-h-0 flex-[0.58] flex-col pt-2" : "flex min-h-0 flex-[0.62] flex-col pt-3"}>
        <p className="text-xs font-semibold text-accent">{category?.title ?? destination}</p>
        <p className={isCompact ? "mt-1.5 text-[0.98rem] font-semibold leading-[1.18] text-ink sm:text-[1.02rem]" : "mt-1.5 text-[1.12rem] font-semibold leading-[1.18] text-ink sm:text-[1.2rem]"}>{product.title}</p>
        <p className={isCompact ? "mt-2 text-sm font-normal text-ink" : "mt-2.5 text-base font-normal text-ink"}>{purchaseLabel}</p>
      </div>
    </Link>
  );
}
