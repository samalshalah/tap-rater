...
September 5, 2026. Engineering changes deployed and tested. **One Phase 4 gate remains: owner-controlled portable encryption-key custody and retrieval.** Stripe remains in test mode; this is not live-launch approval.
...
- Local DPAPI recovery-key backup decrypts and has the expected format. It still requires portable owner vault custody.
...
1. Phase 4: owner selects a password manager/secure vault; securely store the recovery key off this Windows machine and verify independent retrieval. Do not paste it or a vault password into chat.
2. Phase 5: owner-observed rollback/roll-forward, hosted snapshot republishing, inbox/activation/reset, customer portals, and operating-policy acceptance. Automated tests do not close these checks.
3. Phase 6: tax/policy approval, deliberate live configuration, controlled real-money purchases, and payment/order/invoice/subscription/payout reconciliation.
